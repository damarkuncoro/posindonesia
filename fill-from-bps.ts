...
async function fetchBpsVillageCodes(districtCode: string): Promise<BpsVillage[]> {
  ...
  try {
    ...
    const data: BpsVillage[] = await response.json();
    return data;
  } catch (error) {
    console.error(`Error fetching BPS data for district ${districtCode}:`, error);
    return [];
  }
}

// Parse the data file
function parseDataFile(content: string): PostalCodeData[] {
...
function matchVillageName(dataVillage: string, bpsVillage: string): boolean {
  ...
  for (const [short, long] of Object.entries(abbrev)) {
    if (d.replace(short, long) === b || b.replace(short, long) === d) return true;
  }
...
// Main function
async function fillVillageCodesFromBps(): Promise<void> {
  ...
  const districtMap = new Map<string, PostalCodeData[]>();
